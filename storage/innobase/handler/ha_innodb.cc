...
#ifdef WITH_WSREP
	/* If error inserting key rollback increment in num_write_row to avoid
	calling intermediate commit with fake threshold mark. */
	if (error_result && wsrep_on(m_user_thd) &&
	    wsrep_load_data_splitting && sql_command == SQLCOM_LOAD &&
	    !thd_test_options(m_user_thd,
			      OPTION_NOT_AUTOCOMMIT | OPTION_BEGIN)) {
		--m_num_write_row;
	}
